...
// Function to parse the games txt file with robust parsing
function parseGamesFromTxtRobust(filePath) {
  ...
  for (const section of sections) {
    ...
    // Debug section moved to after value parsing
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        const line = lines[i]
        if (line.includes('Concepteur(s)')) {
          const parts = line.split(':')
          if (parts.length >= 2) {
            designers = parts.slice(1).join(':').trim() // In case there are more colons in the value
          }
        } else if (line.includes('Éditeur')) {
          const parts = line.split(':')
          if (parts.length >= 2) {
            editor = parts.slice(1).join(':').trim()
          }
        } else if (line.includes('Joueurs')) {
          const parts = line.split(':')
          if (parts.length >= 2) {
            playersStr = parts.slice(1).join(':').trim()
          }
        } else if (line.includes('Âge')) {
          const parts = line.split(':')
          if (parts.length >= 2) {
        ...
        } else if (line.includes('Mécaniques principales')) {
          const parts = line.split(':')
          if (parts.length >= 2) {
        ...
        } else if (line.includes('Source vérifiable')) {
          const parts = line.split(':')
          if (parts.length >= 2) {
...
async function insertGames() {
  ...
  const games = parseGamesFromTxtRobust(txtFilePath)
