...
// Function to parse the games txt file with fixed parsing
function parseGamesFromTxtFixed(filePath) {
  ...
  for (const section of sections) {
    ...
    try {
      ...
      // Parse subsequent lines - flexible matching with regex for Unicode dashes and spaces
      for (let i = 1; i < lines.length; i++) {
        const line = lines[i]
        const trimmedLine = line.trim()
        if (/^Concepteur\(s\)\s*[:‑]\s*/u.test(trimmedLine)) {
          const parts = line.split(/[:‑]/)
          if (parts.length >= 2) {
            designers = parts.slice(1).join(':').trim()
          }
        } else if (/^Éditeur\s*[:‑]\s*/u.test(trimmedLine)) {
          const parts = line.split(/[:‑]/)
          if (parts.length >= 2) {
            editor = parts.slice(1).join(':').trim()
          }
        } else if (/^Joueurs\s*[:‑]\s*/u.test(trimmedLine)) {
          const parts = line.split(/[:‑]/)
          if (parts.length >= 2) {
            playersStr = parts.slice(1).join(':').trim()
          }
        } else if (/^Âge\s*[:‑]\s*/u.test(trimmedLine)) {
          const parts = line.split(/[:‑]/)
          if (parts.length >= 2) {
            ageStr = parts.slice(1).join(':').trim()
          }
        } else if (/^Durée\s*[:‑]\s*/u.test(trimmedLine)) {
          const parts = line.split(/[:‑]/)
          if (parts.length >= 2) {
            durationStr = parts.slice(1).join(':').trim()
          }
        } else if (/^Mécaniques\s+principales\s*[:‑]\s*/u.test(trimmedLine)) {
          const parts = line.split(/[:‑]/)
          if (parts.length >= 2) {
            mechanics = parts.slice(1).join(':').trim()
          }
        } else if (/^Objectif\s*[:‑]\s*/u.test(trimmedLine)) {
          const parts = line.split(/[:‑]/)
          if (parts.length >= 2) {
            objective = parts.slice(1).join(':').trim()
          }
        } else if (/^Source\s+vérifiable\s*[:‑]\s*/u.test(trimmedLine)) {
          const parts = line.split(/[:‑]/)
          if (parts.length >= 2) {
...
async function insertGames() {
  ...
  const games = parseGamesFromTxtFixed(txtFilePath)
